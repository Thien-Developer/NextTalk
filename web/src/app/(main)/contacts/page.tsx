...
type PublicUser = Pick<User, 'id' | 'displayName' | 'avatarUrl' | 'email'>
...
export default function ContactsPage() {
  ...
  const { data: searchResults } = useQuery({
    ...
    queryFn: async () => (await userApi.searchUsers(search)).data,
  ...
  const respond = useMutation({
    mutationFn: ({ id, action }: { id: string; action: 'accept' | 'reject' }) =>
      userApi.respondFriendRequest(id, action),
    onSuccess: () => {
  ...
  const startChat = useMutation({
    mutationFn: (friendId: string) =>
      chatApi.createConversation({ type: 'direct', memberIds: [friendId] }),
    onSuccess: ({ data }) => router.push(`/chat/${data.id}`),
    onError: () => toast.error('Không thể mở cuộc trò chuyện'),
  })

  const friendIds = new Set<string>(friends?.map((f: PublicUser) => f.id) ?? [])
      ...
      <header className="px-6 py-4 border-b border-border bg-bg-secondary">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Tìm theo email hoặc tên (≥3 ký tự)"
        ...
        {search.length >= 3 && (
          <section>
            ...
            {!searchResults || searchResults.length === 0 ? (
              <p className="text-sm text-text-muted px-3">Không tìm thấy người dùng</p>
          ...
          <section>
            ...
            {requests.map((req: { requesterId: string; requester: PublicUser }) => (
              <div key={req.requesterId} className="flex items-center gap-3 p-3 rounded-xl hover:bg-bg-hover">
                ...
                <div className="flex-1">
                  <p className="font-medium text-text-primary">{req.requester.displayName}</p>
                  <p className="text-xs text-text-muted">{req.requester.email}</p>
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={() => respond.mutate({ id: req.requesterId, action: 'accept' })}
                    disabled={respond.isPending}
                    className="w-8 h-8 rounded-full bg-gold/20 text-gold flex items-center justify-center hover:bg-gold/30"
                  >
                    <Check className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => respond.mutate({ id: req.requesterId, action: 'reject' })}
                    disabled={respond.isPending}
        ...
        <section>
          ...
          {friends?.map((f: PublicUser) => (
            <div key={f.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-bg-hover">
              ...
              <div className="flex-1">
                <p className="font-medium text-text-primary">{f.displayName}</p>
                <p className="text-xs text-text-muted">{f.email}</p>
              </div>
              <button
                onClick={() => startChat.mutate(f.id)}
                disabled={startChat.isPending}
                className="w-8 h-8 rounded-lg bg-bg-hover flex items-center justify-center text-text-muted hover:text-gold"
              >
